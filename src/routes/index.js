...
router.post("/addIncidencia", async (req, res) => {
  try {
    const response = await db.query(
      "INSERT INTO incidencias(tipo,descripcion,fechaEmitida,isResuelta,ubicacion,id_usuario,id_estadio) VALUES($1,$2,NOW(),false,$3,$4,$5)",
      [
        req.body.incidencia.tipo,
        req.body.incidencia.descripcion,
        req.body.incidencia.ubicacion,
        req.body.user.ido,
...
router.get("/getIncidencias", async (req, res) => {
  try {
    const response = await db.query(
      "SELECT i.ubicacion, count (i.ubicacion) FROM incidencias i where i.ubicacion in (select nombre from ubicacion) GROUP BY i.ubicacion"
    );
    res.json(response.rows);
  } catch (error) {
    console.log(error);
